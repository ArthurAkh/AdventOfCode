...
def controllarules(frequenzaprec=None):
    tempfreq=list(map(list,zip([el[0] for el in insRules],[0]*len(insRules))))
...
if __name__ == '__main__':
    ...
    for l in f:
        ...
        if len(x.split(" -> ")) == 1 and len(x) > 0:
            ...
            insRules.append(x.split(" -> "))

    frequenza=list(map(list,zip([el[0] for el in insRules],[0]*len(insRules))))
